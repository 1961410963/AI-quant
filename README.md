...
│   │   ├── ma_batch_analysis.html     #    双均线策略批量回测对比报告（项目04）
│   │   ├── ma_batch_results.csv       #    批量回测结果数据（项目04）
│   │   └── ...其他数据文件
│   ├── scripts/              #    🔧 辅助脚本
...
│   ├── batch_ma_backtest.py     #    双均线策略批量回测与对比分析（项目04）
...
- 📈 **回测指标** — 累计回报、最大回撤（MDD）、夏普比率
- 📉 **可视化** — 股价+均线走势、交易信号标记、资产净值曲线、最大回撤曲线
