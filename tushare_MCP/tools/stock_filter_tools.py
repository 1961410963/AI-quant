...
def register_stock_filter_tools(mcp: "FastMCP"):
    ...
    def collect_stock_data(
        ...
        try:
            ...
            try:
                query = "SELECT COUNT(*) FROM t1_data WHERE trade_date = ?"
                count = pd.read_sql(query, conn, params=[t1_date]).iloc[0, 0]
                if count > 0:
                    conn.close()
            ...
            try:
                query = "SELECT COUNT(*) FROM t1_data WHERE trade_date = ?"
                final_count = pd.read_sql(query, conn, params=[t1_date]).iloc[0, 0]
                ...
                query_stats = """
                    ...
                    WHERE trade_date = ?
                """
                stats = pd.read_sql(query_stats, conn, params=[t1_date]).iloc[0]
                result_parts.append(f"\n数据完整性统计：")
