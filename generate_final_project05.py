...
<div class="container">
    ...
    <div class="section">
        ...
        <div class="fig-caption">
            ...
            绿色（正向系数）的特征会提高上涨概率，红色（负向系数）的特征会降低上涨概率。
            由于逻辑回归是线性模型，系数的可解释性很强，可以直观地看到哪些财务指标对股价涨跌有正向/负向影响。
            但由于整体AUC很低，这些系数的统计显著性需要谨慎解读。
        </div>
    </div>
...
function renderROC() {
    ...
    const series = Object.keys(roc).map((name, i) => {
        const data = roc[name].fpr.map((fpr, idx) => [fpr, roc[name].tpr[idx]]);
        return {
    ...
    chart.setOption({
        tooltip: {
            ...
            formatter: function(params) {
                ...
                params.forEach(function(p) {
                    result += '<span style="display:inline-block;width:10px;height:10px;border-radius:50%;background:' + p.color + ';margin-right:6px;"></span>' + p.seriesName + ': <b>' + p.value[1].toFixed(4) + '</b><br/>';
                });
                return result;
            }
        },
        legend: { bottom: 5 },
        grid: { left: 70, right: 180, top: 20, bottom: 60 },
        ...
        visualMap: {
            ...
            type: 'piecewise',
            pieces: [
                { min: 0.9, max: 1.0, label: '0.9~1.0 完美分类', color: '#047857' },
                { min: 0.8, max: 0.9, label: '0.8~0.9 模型良好', color: '#10b981' },
                { min: 0.7, max: 0.8, label: '0.7~0.8 模型有效', color: '#f59e0b' },
                { min: 0.5, max: 0.7, label: '0.5~0.7 随机猜测', color: '#dc2626' }
            ],
            right: 20,
            top: 'center',
            itemWidth: 24,
