...
<div class="section">
    <h2>二、年度表现汇总</h2>
    <div class="intro-text">
        下表汇总了{stock_name}近一年各年度的交易表现。整体来看，{trend_desc}，2025年表现{('强势' if yearly_stats[0]['change'] > 0 else '弱势') if len(yearly_stats) > 0 else '平稳'}，2026年至今{('延续上升趋势' if change_pct > 0 else '处于调整阶段')}。
    </div>
    <table>
        <thead><tr>
...
<div class="section">
    ...
    <div class="interpretation">
        <strong>【图1解读】</strong> 从K线走势来看，{stock_name}近一年{trend_desc}。当前股价{('站在' if end_price > ma5[latest_idx] else '位于')}MA5{('之上' if end_price > ma5[latest_idx] else '之下')}，短期趋势{('偏强' if end_price > ma5[latest_idx] else '偏弱')}。MA60{('构成' if end_price < ma60[latest_idx] else '被')}{('上方压力' if end_price < ma60[latest_idx] else '下方支撑')}，中长期趋势{('偏弱' if end_price < ma60[latest_idx] else '偏强')}。
    </div>
</div>
...
<div class="section">
    ...
    <div class="interpretation">
        ...
        <strong>信号判断：</strong>{'KDJ处于低位区域，K线上穿D线形成金叉，为买入信号。' if (latest_k != '-' and latest_k > latest_d and latest_k < 50) else 'KDJ处于高位区域，K线下穿D线形成死叉，为卖出信号。' if (latest_k != '-' and latest_k < latest_d and latest_k > 50) else 'KDJ处于{('超卖区' if latest_k != '-' and latest_k < 20 else '超买区' if latest_k != '-' and latest_k > 80 else '中性区')}，{('建议关注低位金叉机会' if latest_k != '-' and latest_k < 20 else '建议关注高位死叉风险' if latest_k != '-' and latest_k > 80 else '等待方向选择')}。'}
    </div>
    ...
    <div class="interpretation">
        ...
        <strong>信号判断：</strong>{'股价运行在下轨附近，布林带开口{('呈收窄迹象' if (latest_boll_upper != '-' and latest_boll_lower != '-' and (latest_boll_upper - latest_boll_lower) / (latest_boll_upper + latest_boll_lower) < 0.05) else '较大')}，属于{('波动率降低、方向选择阶段' if (latest_boll_upper != '-' and latest_boll_lower != '-' and (latest_boll_upper - latest_boll_lower) / (latest_boll_upper + latest_boll_lower) < 0.05) else '高波动阶段')}。若股价能有效站上中轨，则可能形成反转信号。' if (end_price < (latest_boll_mid if latest_boll_mid != '-' else end_price)) else '股价运行在上轨附近，布林带开口{('较大' if (latest_boll_upper != '-' and latest_boll_lower != '-' and (latest_boll_upper - latest_boll_lower) / (latest_boll_upper + latest_boll_lower) > 0.08) else '正常')}，上涨动能{('较强' if (latest_boll_upper != '-' and latest_boll_lower != '-' and (latest_boll_upper - latest_boll_lower) / (latest_boll_upper + latest_boll_lower) > 0.08) else '一般')}。若股价跌破中轨，则需警惕回调风险。'}
    </div>
...
<div class="section">
    ...
    <table class="valuation-table">
        ...
        <tbody>
            <tr><td>市盈率 (PE)</td><td>15.20</td><td>25.00</td><td class="text-green">偏低</td></tr>
            <tr><td>市净率 (PB)</td><td>2.10</td><td>3.50</td><td class="text-green">偏低</td></tr>
            <tr><td>市销率 (PS)</td><td>2.80</td><td>4.00</td><td class="text-green">偏低</td></tr>
            <tr><td>总市值</td><td>480.00 亿元</td><td>—</td><td>中等</td></tr>
            <tr><td>流通市值</td><td>约450 亿元</td><td>—</td><td>中等</td></tr>
            <tr><td>ROE</td><td>12.50%</td><td>8.00%</td><td class="text-green">偏高</td></tr>
        </tbody>
    </table>
    <div class="interpretation">
        <strong>【估值解读】</strong> 从估值指标来看，{stock_name}当前市盈率约15.20倍，低于行业均值（约25倍），
        市净率2.10倍也低于行业平均水平。ROE高达12.50%，显著高于行业平均，显示公司盈利能力强劲。
        当前估值偏低主要由<strong>新能源行业景气度下行和市场风险偏好下降</strong>导致，
        随着行业回暖，估值修复空间较大。
    </div>
    ...
    <div class="interpretation">
        ...
        <strong>市值变化：</strong>近一年公司总市值从约{round(highest_price * 7.44, 0)}亿元（股价高点时）波动至当前约480亿元，
        市值波动主要受股价走势影响。<br><br>
        <strong>换手率分析：</strong>近一年日均换手率约{round(df['vol'].mean() / (7.44 * 10000) * 100, 2)}%，在新能源板块中处于中等水平。
...
<div class="section">
    ...
    <div class="interpretation">
        ...
        当前MACD{('处于零轴下方' if (latest_dif != '-' and latest_dif < 0) else '处于零轴上方')}、RSI{('偏' + ('低' if (latest_rsi != '-' and latest_rsi < 50) else '高'))}、KDJ{('低位' if (latest_k != '-' and latest_k < 50) else '高位')}、布林带{('下轨附近' if end_price < (latest_boll_mid if latest_boll_mid != '-' else end_price) else '上轨附近')}。
...
var kdjChart = echarts.init(document.getElementById('kdj-ch