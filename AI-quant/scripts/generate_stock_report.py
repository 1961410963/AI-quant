...
def get_stock_data(ts_code, days=365, output_dir='output'):
    ...
    try:
        ...
        if not token:
            raise ValueError("未配置 TUSHARE_TOKEN 环境变量。请设置后重试。")
        ts.set_token(token)
        pro = ts.pro_api()
...
<div class="section">
    ...
    <div class="interpretation">
        <strong>【图3解读】</strong>MACD（指数平滑异同移动平均线）由DIF线（快线）、DEA线（慢线）和MACD柱（DIF-DEA）组成，用于判断趋势强弱和多空转换。本站MACD柱不乘2，直接等于DIF减DEA。<br><br>
    ...
    <div class="interpretation">
        <strong>【图7解读】</strong>ATR（Average True Range，平均真实波幅）衡量价格波动幅度，不判断方向。TR取三个值的最大值：当日高低差、高与昨收差的绝对值、低与昨收差的绝对值，比"最高-最低"更全面地反映了隔夜跳空带来的价格波动。本站采用14日Wilder平滑方法。<br><br>
...
klineChart.setOption({{
    ...
    tooltip: {{
        ...
        formatter: function(params) {{
            ...
            return '<div style="font-weight:700;color:#1f2937;border-bottom:1px solid #e5e7eb;padding-bottom:6px;margin-bottom:6px;">' + dates[idx] + '</div>' +
                '<div style="display:flex;justify-content:space-between;gap:20px;"><span style="color:#6b7280;">开盘</span><span style="font-weight:600;">' + Number(d[0]).toFixed(2) + '</span></div>' +
                '<div style="display:flex;justify-content:space-between;gap:20px;"><span style="color:#6b7280;">收盘</span><span style="font-weight:600;color:' + color + '">' + Number(d[1]).toFixed(2) + '</span></div>' +
                '<div style="display:flex;justify-content:space-between;gap:20px;"><span style="color:#6b7280;">最高</span><span style="font-weight:600;">' + Number(d[3]).toFixed(2) + '</span></div>' +
                '<div style="display:flex;justify-content:space-between;gap:20px;"><span style="color:#6b7280;">最低</span><span style="font-weight:600;">' + Number(d[2]).toFixed(2) + '</span></div>' +
    ...
    xAxis: commonXAxis,
    yAxis: {{ type: 'value', scale: true, axisLine: {{ lineStyle: {{ color: '#9ca3af' }} }}, axisLabel: {{ color: '#6b7280', fontSize: 11, formatter: function(v){{ return Number(v).toFixed(2); }} }}, splitLine: {{ lineStyle: {{ color: '#f3f4f6', type: 'dashed' }} }} }},
...
var volumeChart = echarts.init(document.getElementById('volume-chart2'));
volumeChart.setOption({{
    ...
    yAxis: {{ type: 'value', axisLine: {{ lineStyle: {{ color: '#9ca3af' }} }}, axisLabel: {{ color: '#6b7280', fontSize: 11, formatter: function(v){{ return Number(v).toFixed(2); }} }}, splitLine: {{ lineStyle: {{ color: '#f3f4f6', type: 'dashed' }} }} }},
    dataZoom: commonDataZoom,
...
rsiChart.setOption({{
    ...
    grid: {{ left: '6%', right: '3%', top: '15%', bottom: '15%' }},
    xAxis: commonXAxis,
    yAxis: {{ type: 'value', min: 0, max: 100, axisLine: {{ lineStyle: {{ color: '#9ca3af' }} }}, axisLabel: {{ color: '#6b7280', fontSize: 11, formatter: function(v){{ return Number(v).toFixed(2); }} }}, splitLine: {{ lineStyle: {{ color: '#f3f4f6', type: 'dashed' }} }} }},
    dataZoom: commonDataZoom,
    series: [
...
turnoverChart.setOption({{
    ...
    xAxis: commonXAxis,
    yAxis: [
        {{ type: 'value', name: '市值(亿元)', axisLine: {{ lineStyle: {{ color: '#3b82f6' }} }}, axisLabel: {{ color: '#3b82f6', fontSize: 11, formatter: function(v){{ return Number(v).toFixed(2); }} }}, splitLine: {{ lineStyle: {{ color: '#f3f4f6', type: 'dashed' }} }} }},
        {{ type: 'value', name: '换手率(%)', axisLine: {{ lineStyle: {{ color: '#f97316' }} }}, axisLabel: {{ color: '#f97316', fontSize: 11, formatter: function(v){{ return Number(v).toFixed(2); }} }}, splitLine: {{ show: false }} }}
    ],
    dataZoom: commonDataZoom,
    series: [
...
echarts.connect([klineChart, volumeChart, macdChart, rsiChart, kdjChart, bollChart, atrChart, turnoverChart]);
